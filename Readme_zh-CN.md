...
示例代码：[split.mjs](./examples/split.mjs)
...
示例代码：[string-util.mjs](./examples/string-util.mjs)
...
示例代码：[options-common.mjs](./examples/options-common.mjs)
...
示例代码：[options-defaults.mjs](./examples/options-defaults.mjs)
...
示例代码：[options-negatable.mjs](./examples/options-negatable.mjs)
...
示例代码：[options-boolean-or-value.mjs](./examples/options-boolean-or-value.mjs)
...
示例代码：[options-required.mjs](./examples/options-required.mjs)
...
示例代码：[options-variadic.mjs](./examples/options-variadic.mjs)
...
示例代码：[options-extra.mjs](./examples/options-extra.mjs), [options-env.mjs](./examples/options-env.mjs), [options-conflicts.mjs](./examples/options-conflicts.mjs), [options-implies.mjs](./examples/options-implies.mjs)
...
示例代码：[options-custom-processing.mjs](./examples/options-custom-processing.mjs)
...
通过`.command()`或`.addCommand()`可以配置命令，有两种实现方式：为命令绑定处理函数，或者将命令单独写成一个可执行文件（详述见后文）。子命令支持嵌套（[示例代码](./examples/nestedCommands.mjs)）。
...
使用`.command()`和`addCommand()`来指定选项的相关设置。当设置`hidden: true`时，该命令不会打印在帮助信息里。当设置`isDefault: true`时，若没有指定其他子命令，则会默认执行这个命令（[样例](./examples/defaultCommand.mjs)）。
...
示例代码：[argument.mjs](./examples/argument.mjs)
...
示例代码：[arguments-extra.mjs](./examples/arguments-extra.mjs)
...
示例代码：[arguments-custom-processing.mjs](./examples/arguments-custom-processing.mjs)
...
示例代码：[thank.mjs](./examples/thank.mjs)
...
示例代码：[action-this.mjs](./examples/action-this.mjs)
...
示例代码：[hook.mjs](./examples/hook.mjs)
...
示例代码：[configure-help.mjs](./examples/configure-help.mjs)
...
示例代码：[positional-options.mjs](./examples/positional-options.mjs)
...
示例代码：[pass-through-options.mjs](./examples/pass-through-options.mjs)
...
`createCommand`同时也是`Command`对象的一个方法，可以创建一个新的命令（而非子命令），使用`.command()`创建子命令时内部会调用该方法，具体使用方式可参考 [custom-command-class.mjs](./examples/custom-command-class.mjs)。
...
示例代码：[configure-output.mjs](./examples/configure-output.mjs)
