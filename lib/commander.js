...
/**
 * If you want to use version property from package.json or from filename
 *
 * @param {String} failVersion if can't find package.json or filename 
 * @param {String} filename if different from package.json
 * @return {String} version from filename
 * @api public
 */
Command.prototype.packageVersion = function(failVersion, filename) {
  var path = require('path'),
      fs = require('fs');
  if(!filename) filename = __dirname + '/../package.json';
  if (path.existsSync(filename)) {
    return JSON.parse(fs.readFileSync(filename)).version;
  }

  if(failVersion) return failVersion;
};
...
Command.prototype.password = function(str, mask, fn){
  ...
  process.stdin.on('keypress', function(c, key){
    ...
    if (key && 'backspace' == key.name && mask) {
      var len = buf.length-1;
      if(len >= 0) {
        buf = buf.substr(0, len);
        // send ascii code to go back to previous position
        // and clear everything at the right of the cursor position
        process.stdout.write('\u001B[1D\u001B[K');
      }
      return;
    }
