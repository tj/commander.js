...
class Help {
  ...
  _visibleThingGroups(cmd, helper, info) {
    ...
    const visibleThings = info.visibleThings;
    ...
    info.overrideMap.forEach((ids, group) => {
      addGroup(group);
      ids.forEach(id => {
        const index = visibleThings.findIndex(thing => info.match(id, thing));
        if (index >= 0) {
          groupMap.get(group).push(visibleThings[index]);
          visibleThings.splice(index, 1);
        }
      });
    });

    // Scan for new groups in order commands created.
    info.things.forEach(sub => addGroup(sub.group()));
  ...
  _visibleCommandGroups(cmd, helper) {
    return this._visibleThingGroups(cmd, helper, {
      ...
      overrideMap: helper.commandGroups,
      things: cmd.commands,
  ...
  _visibleOptionGroups(cmd, helper) {
    return this._visibleThingGroups(cmd, helper, {
      ...
      overrideMap: helper.optionGroups,
      things: cmd.options,
