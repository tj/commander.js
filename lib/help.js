...
class Help {
  ...
  _visibleCommandGroups(cmd, helper) {
    ...
    // Process explicit groups from helper first.
    helper.commandGroups.forEach((names, group) => {
      addGroup(group);
      names.forEach(name => {
        const index = subs.findIndex(sub => name === sub.name());
        if (index >= 0) {
          groupMap.get(group).push(subs[index]);
          subs.splice(index, 1);
        }
      });
    });

    // Scan for new groups in order commands created.
    cmd.commands.forEach(sub => addGroup(sub.group()));
    addGroup(defaultTitle);
    // Populate groups with remaining subcommands.
    subs.forEach(sub => {
      const group = sub.group() ?? defaultTitle;
      groupMap.get(group).push(sub);
    });
    return groupMap;
  }
  ...
  _visibleOptionGroups(cmd, helper) {
    ...
    // Process explicit groups from helper first.
    helper.optionGroups.forEach((flags, group) => {
      addGroup(group);
      flags.forEach(flag => {
        const index = opts.findIndex(opt => opt.is(flag) || flag === opt.attributeName());
        if (index >= 0) {
          groupMap.get(group).push(opts[index]);
          opts.splice(index, 1);
        }
      });
    });

    // Scan for new groups in order options created.
    cmd.options.forEach(opt => addGroup(opt.helpGroup));
    addGroup(defaultTitle);
    // Populate groups with remaining subcommands.
    opts.forEach(opt => {
      const group = opt.helpGroup ?? defaultTitle;
