 ...
 - if the default executable name is not suitable, use the executableFile option to supply a custom name`;
  ...
  unknownOption(flag) {
    ...
    if (flag.startsWith('--')) {
      ...
      if (this._hasHelpOption && this._helpLongFlag) candidateFlags.push(this._helpLongFlag);
      let command = this;
      do {
        const moreFlags = command.options
          .filter(option => option.long)
          .map(option => option.long);
        if (moreFlags.length > 0) {
          candidateFlags.push(...moreFlags);
        }
  ...
  unknownCommand() {
    ...
    this.commands.forEach((command) => {
      ...
      const alias = command.alias(); // Just visible alias
      if (alias) candidateNames.push(alias);
    });
    if (this._hasImplicitHelpCommand()) candidateNames.push(this._helpCommandName);
