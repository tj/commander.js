...
const { findSimilar } = require('./distance');
 ...
 - if the default executable name is not suitable, use the executableFile option to supply a custom name`;
  ...
  unknownCommand() {
    ...
    this.commands.forEach((command) => {
      if (command._name.length > 1) exactNames.push(command._name);
      if (command._aliases) aliasedNames.push(...command._aliases);
    });
    if (this._hasImplicitHelpCommand()) exactNames.push(this._helpCommandName);
    const candidateNames = exactNames.concat(aliasedNames);
    const similarCommand = findSimilar(unknownName, candidateNames);
    if (similarCommand) {
