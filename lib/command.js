...
class Command extends EventEmitter {
  ...
  parse(argv) {
    ...
    // Look for subcommand
    if (name) {
      subCommand = this.commands.find((command) => {
  ...
  executeSubCommand(argv, args, unknown, executableFile) {
    ...
    if (!this._exitCallback) {
      ...
      proc.on('close', () => {
        this._exitCallback(new CommanderError(process.exitCode || 0, 'commander.executeSubCommandAsync', '(close)'));
      });
    }
    proc.on('error', (err) => {
      if (err.code === 'ENOENT') {
      ...
      if (!this._exitCallback) {
        ...
        this._exitCallback(wrappedError);
      }
    });
  ...
  parseArgs(args, unknown) {
    ...
    if (args.length) {
      ...
      if (this.commands.length === 0 &&
        this._args.filter((a) => { return a.required; }).length === 0) {
        this.emit('command:*');
      }
    }
