...
program.commands[0]._data.should.not.have.property.setup_mode;
program.commands[1]._data.should.not.have.property.exec_mode;
...
program.commands[0].get('setup_mode').should.equal("mode3");
program.commands[0]._data.should.not.have.property.host;
envValue.should.equal("env1");

program.parse(['node', 'test', '--config', 'conf2', 'setup', '--setup_mode', 'mode3', '-o', 'host1', 'env2']);
program.get('config').should.equal("conf2");
program.commands[0].get('setup_mode').should.equal("mode3");
program.commands[0].get('host').should.equal("host1");
envValue.should.equal("env2");

program.parse(['node', 'test', '--config', 'conf3', 'setup', '-s', 'mode4', 'env3']);
program.get('config').should.equal("conf3");
program.commands[0].get('setup_mode').should.equal("mode4");
envValue.should.equal("env3");

program.parse(['node', 'test', '--config', 'conf4', 'exec', '--exec_mode', 'mode1', 'exec1']);
program.get('config').should.equal("conf4");
program.commands[1].get('exec_mode').should.equal("mode1");
program.commands[1]._data.should.not.have.property.target;
cmdValue.should.equal("exec1");

program.parse(['node', 'test', '--config', 'conf5', 'exec', '-e', 'mode2', 'exec2']);
program.get('config').should.equal("conf5");
program.commands[1].get('exec_mode').should.equal("mode2");
cmdValue.should.equal("exec2");

program.parse(['node', 'test', '--config', 'conf6', 'exec', '--target', 'target1', '-e', 'mode2', 'exec3']);
program.get('config').should.equal("conf6");
program.commands[1].get('exec_mode').should.equal("mode2");
program.commands[1].get('target').should.equal("target1");
cmdValue.should.equal("exec3");

program.parse(['node', 'test', '--config', 'conf7', 'ex', '-e', 'mode3', 'exec4']);
program.get('config').should.equal("conf7");
program.commands[1].get('exec_mode').should.equal("mode3");
program.commands[1]._data.should.not.have.property.target;
