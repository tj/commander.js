...
test('SGR only', () => {
  const str = `${CSI}4m`;
  expect(stripAnsi(str)).toEqual('');
});

test('SGR at start', () => {
  const str = `${CSI}4mX`;
  expect(stripAnsi(str)).toEqual('X');
});

test('SGR in middle', () => {
  const str = `X${CSI}4mY`;
  expect(stripAnsi(str)).toEqual('XY');
});

test('SGR at end', () => {
  const str = `${CSI}4mY`;
  expect(stripAnsi(str)).toEqual('Y');
});

test('SGR pair', () => {
  // underline and not underlined
  const str = `${CSI}4mABC${CSI}24m`;
  expect(stripAnsi(str)).toEqual('ABC');
});
...
test('incomplete SGR sequence', () => {
  const str = `${CSI}14X`;
  expect(stripAnsi(str)).toEqual(`${CSI}14X`);
});
